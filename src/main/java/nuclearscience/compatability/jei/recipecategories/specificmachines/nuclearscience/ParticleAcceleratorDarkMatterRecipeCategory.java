...
public class ParticleAcceleratorDarkMatterRecipeCategory implements IRecipeCategory<O2OProcessingRecipe> {
	...
	cachedArrows = CacheBuilder.newBuilder().maximumSize(25).build(new CacheLoader<Integer, ArrayList<IDrawableAnimated>>() {
		...
		IDrawableAnimated majorArrowBottom = guiHelper.drawableBuilder(GUI_TEXTURE, 179, 17, 37, 75).buildAnimated(cookTime,
			IDrawableAnimated.StartDirection.BOTTOM, false);

		IDrawableAnimated majorArrowTop = guiHelper.drawableBuilder(GUI_TEXTURE, 132, 0, 37, 75).buildAnimated(cookTime,
