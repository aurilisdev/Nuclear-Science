...
public class WrapperQuantumTunnelFrequencies {
    ...
    public WrapperQuantumTunnelFrequencies(ScreenQuantumTunnel screen, int x, int y) {
        ...
        frequencyLabel = new ScreenComponentSimpleLabel(x + 6, y + 25, 10, Color.WHITE, () -> {
            ...
            ITextComponent frequencyName;
            if (selectedFrequency != null) {
                frequencyName = new StringTextComponent(selectedFrequency.getName());
                ...
                frequencyName = new StringTextComponent(tile.frequency.getValue().getName());
            }
        ...
        for (int i = 0; i < BUTTON_COUNT; i++) {
            ...
            }).onTooltip((poseStack, button, xAxis, yAxis) -> screen.displayTooltip(poseStack, NuclearTextUtils.gui("quantumtunnel.edit").getVisualOrderText(), xAxis, yAxis)).setIcon(NuclearIconTypes.PENCIL);
        }
        ...
        for (int i = 0; i < BUTTON_COUNT; i++) {
            ...
            deleteButtons[i] = (ScreenComponentButton) new ScreenComponentButton<>(x + butOffX + 132, y + 2 + butOffY + 25 * i,20, 20).setOnPress(but -> {
                ...
                NetworkHandler.CHANNEL.sendToServer(new PacketDeleteFrequency(player.getUUID(), frequency));



            }).onTooltip((poseStack, button, xAxis, yAxis) -> screen.displayTooltip(poseStack, NuclearTextUtils.gui("quantumtunnel.delete").getVisualOrderText(), xAxis, yAxis)).setIcon(NuclearIconTypes.DELETE);
        }
    ...
    public void tick() {
        ...
        if (lastRowCount > BUTTON_COUNT) {
            ...
            if (!slider.isSliderHeld()) {
                int moveRoom = screen.slider.getHeight() - 15 -2;
