...
public class ButtonTunnelFrequency extends ScreenComponentButton<ButtonTunnelFrequency> {
    ...
    public void renderBackground(PoseStack poseStack, int xAxis, int yAxis, int guiWidth, int guiHeight) {
        ...
        screen.getFontRenderer().draw(poseStack, Component.literal(frequency.getName()), guiWidth + xLocation + 5, guiHeight + yLocation + 5, Color.WHITE.color());

        Player player = tile.getLevel().getPlayerByUUID(frequency.getCreatorId());

        screen.getFontRenderer().draw(poseStack, player != null ? player.getName() : Component.literal(frequency.getCreatorFallbackName()), guiWidth + xLocation + 5, guiHeight + yLocation + 15, Color.TEXT_GRAY.color());
