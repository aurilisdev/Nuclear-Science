...
public class WrapperBindInterface {
    ...
    public WrapperBindInterface(GenericInterfaceBoundScreen<?> screen, int x, int y, int tabX, int tabY,
	    boolean needsButton, boolean updateSlots) {
	this.screen = screen;
	...
	if (needsButton) {
	    screen.addComponent(button = (ScreenComponentButton<?>) new ScreenComponentButton<>(
			...
			tooltips.add(NuclearTextUtils.tooltip("logisticsnetwork.linkinterface")
	...
	label = new ScreenComponentSimpleLabel(x + 10, y + 45, 10, Color.TEXT_GRAY,
	...
	for (int i = 0; i < BUTTON_COUNT; i++) {
	    interfaceButtons[i] = new ButtonInterfaceType(x + butOffX, y + butOffY + 20 * i, 139, 20)
		    .setOnPress(but -> {
