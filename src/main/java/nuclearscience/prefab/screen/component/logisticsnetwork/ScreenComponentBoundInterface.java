...
public class ScreenComponentBoundInterface extends ScreenComponentGeneric {
    ...
    public void renderBackground(MatrixStack poseStack, int xAxis, int yAxis, int guiWidth, int guiHeight) {
        ...
        Minecraft.getInstance().getItemRenderer().renderGuiItem(GenericTileInterface.getItemFromType(inter.type()), guiWidth + x + 2, guiHeight + y + 2);

        FontRenderer font = screen.getFontRenderer();

        ITextComponent text = new StringTextComponent(inter.pos().toShortString());
