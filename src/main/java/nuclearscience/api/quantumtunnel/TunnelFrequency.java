...
public class TunnelFrequency {
	...
	public TunnelFrequency decode(ByteBuf buffer) {
	    return new TunnelFrequency(StreamCodec.UUID.decode(buffer), StreamCodec.UUID.decode(buffer),
		    StreamCodec.STRING.decode(buffer), StreamCodec.STRING.decode(buffer),
		    StreamCodec.INT.decode(buffer));
	}
    };
