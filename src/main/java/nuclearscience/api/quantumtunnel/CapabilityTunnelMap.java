...
public class CapabilityTunnelMap implements ICapabilityTunnelMap, ICapabilitySerializable<CompoundTag> {
	...
	for (Map.Entry<TunnelFrequency, TunnelFrequencyBuffer> entry : map.entrySet()) {
	    ...
	    TunnelFrequency.CODEC.encodeStart(NbtOps.INSTANCE, entry.getKey()).result()
		    .ifPresent(tag -> store.put("id", tag));
	    TunnelFrequencyBuffer.CODEC.encodeStart(NbtOps.INSTANCE, entry.getValue()).result()
		    .ifPresent(tag -> store.put("buffer", tag));
	    data.put(i + "", store);
	    i++;
	}
	return data;
	...
	for (int i = 0; i < size; i++) {
	    ...
	    Optional<Pair<TunnelFrequency, Tag>> freq = TunnelFrequency.CODEC.decode(NbtOps.INSTANCE, stored.get("id"))
		    .result();
	    Optional<Pair<TunnelFrequencyBuffer, Tag>> val = TunnelFrequencyBuffer.CODEC
		    .decode(NbtOps.INSTANCE, stored.getCompound("buffer")).result();
