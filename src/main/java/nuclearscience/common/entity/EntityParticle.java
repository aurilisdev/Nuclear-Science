...
public class EntityParticle extends Entity {
    ...
    private static final DataParameter<Integer> TICKS_ALIVE = EntityDataManager.defineId(EntityParticle.class, DataSerializers.INT);
    private static final DataParameter<BlockPos> SOURCE = EntityDataManager.defineId(EntityParticle.class, DataSerializers.BLOCK_POS);
    private static final DataParameter<Boolean> PASSED_THROUGH_GATEWAY = EntityDataManager.defineId(EntityParticle.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> PASSED_THROUGH_SWITCH = EntityDataManager.defineId(EntityParticle.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Direction> SWITCH_DIRECTION = EntityDataManager.defineId(EntityParticle.class, DataSerializers.DIRECTION);
    ...
    public void tick() {
        ...
        if (ticksAlive > NuclearConstants.PARTICLE_SURVIVAL_TICKS) {
            if(isServerside) {
                ...
                level.explode(this, getX(), getY(), getZ(), speed, Mode.BREAK);
            }
            return;
        }
        ...
        TileEntity blockEntity = level.getBlockEntity(source);
        ...
        while (i < checks) {
            ...
            Vector3d proposedMove = new Vector3d(getX() + facingDirection.getStepX() * localSpeed, getY(), getZ() + facingDirection.getStepZ() * localSpeed);
            ...
            if (startOfLoopStateIsBooster) {
                ...
                if (boosterFacing == facingDirection) {
                    ...
                    proposedMove = new Vector3d(floor.getX() + 0.5, floor.getY() + 0.5, floor.getZ() + 0.5);
            ...
            if ((startOfLoopStateIsBooster || startofLoopStateIsDiode) && !passedThroughSwitch) {
                ...
                if (isSwitch(level.getBlockState(proposedMoveBlockPos))) {
                    ...
                    if (switchDirection != facingDirection) {
                        ...
                        proposedMove = new Vector3d(proposedMoveBlockPos.getX() + 0.5, proposedMoveBlockPos.getY() + 0.5, proposedMoveBlockPos.getZ() + 0.5);
                    }
            ...
            if (proposedMoveState.isAir(level, proposedMoveBlockPos) || isSwitch(proposedMoveState) || isGateway(proposedMoveState) || isDiode(proposedMoveState)) {
                ...
                if (canPassThroughGateway && !passedThroughGate) {
                    level.playSound(null, proposedMoveBlockPos, SoundEvents.IRON_TRAPDOOR_OPEN, SoundCategory.BLOCKS, 1.0F, 1.0F);
            ...
            setPos(proposedMove.x, proposedMove.y, proposedMove.z);
    ...
    public boolean canPassThroughGateway(TileEntity entity, BlockState state) {
        return isGateway(state) && entity instanceof TileElectromagneticGateway && ((TileElectromagneticGateway) entity).mayPassThrough(speed);
    }
    ...
    protected void readAdditionalSaveData(CompoundNBT compound) {
    	BlockPos.CODEC.decode(NBTDynamicOps.INSTANCE, compound.get("injector")).result().ifPresent(pair -> source = pair.getFirst());
    ...
    protected void addAdditionalSaveData(CompoundNBT compound) {
    	BlockPos.CODEC.encodeStart(NBTDynamicOps.INSTANCE, source).result().ifPresent(tag -> compound.put("injector", tag));
