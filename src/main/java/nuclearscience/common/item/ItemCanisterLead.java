...
public class ItemCanisterLead extends ItemCanister {
	...
	public Pair<List<ResourceLocation>, List<Fluid>> getWhitelistedFluids() {
		...
		for (Fluid fluid : ForgeRegistries.FLUIDS.getValues()) {
			if (fluid.getBucket() != null && DeferredRegisters.ITEM_CANISTERLEAD.get() != null
					&& fluid.getBucket().getRegistryName().equals(DeferredRegisters.ITEM_CANISTERLEAD.get().getRegistryName())) {
				whitelist.add(fluid);
			}
		}
