...
public class MoltenSaltNetwork
     ...
     * networkMaxTransfer) { return false; }
     * 
     * HashSet<SubtypeMoltenSaltPipe> checkList = new HashSet<>(); for
     * (SubtypeMoltenSaltPipe type : SubtypeMoltenSaltPipe.values()) { if
     * (type.maxTransfer <= attemptSend) { checkList.add(type); } } for
     * (SubtypeMoltenSaltPipe index : checkList) { for (TileMoltenSaltPipe conductor
     * : conductorTypeMap.get(index)) { conductor.destroyViolently(); } } return
     * true; }
     * 
     */
