...
public class TileQuantumTunnel extends GenericTile {
	...
	public TileQuantumTunnel() {
		super(NuclearScienceTiles.TILE_QUANTUMCAPACITOR.get());
	...
	public void tickServer(ComponentTickable tickable) {
		...
		for (Direction direction : readOutputDirections()) {
			...
			TileEntity tile = output.getSafe();
	...
	private int removeDirection(int directions, Direction dir) {
		switch (dir) {
		case DOWN:
			return directions & ~DOWN_MASK;
		case UP:
			return directions & ~UP_MASK;
		case NORTH:
			return directions & ~NORTH_MASK;
		case SOUTH:
			return directions & ~SOUTH_MASK;
		case WEST:
			return directions & ~WEST_MASK;
		case EAST:
			return directions & ~EAST_MASK;
		}
		return 0;
	}
