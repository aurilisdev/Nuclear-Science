...
public class TileAtomicAssembler extends GenericTile {
	...
	public TileAtomicAssembler() {
		super(NuclearScienceTiles.TILE_ATOMICASSEMBLER.get());
		...
		addComponent(new ComponentInventory(this, ComponentInventory.InventoryBuilder.newInv().inputs(7).outputs(1)).setSlotsByDirection(BlockEntityUtils.MachineDirection.TOP, 0, 1, 2, 3, 4, 5).setDirectionsBySlot(6, BlockEntityUtils.MachineDirection.RIGHT, BlockEntityUtils.MachineDirection.BACK)
				//
				.setDirectionsBySlot(7, BlockEntityUtils.MachineDirection.LEFT, BlockEntityUtils.MachineDirection.FRONT).valid((slot, stack, i) -> slot == 6 || slot < 6 && stack.getItem() == NuclearScienceItems.ITEM_CELLDARKMATTER.get()));
	...
	private boolean validateDupeItem(ItemStack stack) {
		...
		if (AtomicAssemblerBlacklistRegister.INSTANCE.isBlacklisted(stack.getItem()) || stack.hasTag() || ItemUtils.testItems(stack.getItem(), NuclearScienceItems.ITEM_CELLDARKMATTER.get()) && stack.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).orElse(CapabilityUtils.EMPTY_ITEM_HANDLER) != CapabilityUtils.EMPTY_ITEM_HANDLER) {
			return false;
		}
