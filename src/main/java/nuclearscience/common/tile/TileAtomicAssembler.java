...
public class TileAtomicAssembler extends GenericTile {
	...
	addComponent(new ComponentElectrodynamic(this, false, true)
		...
		.voltage(NuclearConstants.ATOMICASSEMBLER_VOLTAGE)
	...
	addComponent(new ComponentInventory(this, ComponentInventory.InventoryBuilder.newInv().inputs(7).outputs(1))
		.setSlotsByDirection(BlockEntityUtils.MachineDirection.TOP, 0, 1, 2, 3, 4, 5)
		.setDirectionsBySlot(6, BlockEntityUtils.MachineDirection.RIGHT, BlockEntityUtils.MachineDirection.BACK)
		//
		.setDirectionsBySlot(7, BlockEntityUtils.MachineDirection.LEFT, BlockEntityUtils.MachineDirection.FRONT)
		.valid((slot, stack, i) -> slot == 6
	...
	RadiationUtils.handleRadioactiveItems(this, inv, NuclearConstants.ATOMIC_ASSEMBLER_RADIATION_RADIUS, true, 30,
		true, false);
	...
	boolean validItem = validateDupeItem(input) && (output.isEmpty()
		|| ItemStack.isSameItemSameTags(input, output) && output.getCount() + 1 <= output.getMaxStackSize());
