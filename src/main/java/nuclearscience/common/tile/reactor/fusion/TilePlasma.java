...
public class TilePlasma extends GenericTile {
	...
	public final SingleProperty<Integer> ticksExisted = property(new SingleProperty<>(PropertyTypes.INTEGER, "existed", 0).setNoUpdateClient());
	public final SingleProperty<Integer> spread = property(new SingleProperty<>(PropertyTypes.INTEGER, "spread", 6).setNoUpdateClient());

	private CachedTileOutput output;

	public TilePlasma() {
		super(NuclearScienceTiles.TILE_PLASMA.get());
	...
	public void tickServer(ComponentTickable tickable) {
		...
		if (ticksExisted.getValue() > 1 && level.getBlockState(getBlockPos().relative(Direction.UP)).is(NuclearScienceTags.Blocks.FUSION_CONTAINMENT) && level.getFluidState(getBlockPos().relative(Direction.UP, 2)).is(FluidTags.WATER)) {
			if (output == null) {
