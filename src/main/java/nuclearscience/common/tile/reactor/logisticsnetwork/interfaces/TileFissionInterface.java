...
public class TileFissionInterface extends GenericTileInterface implements IFissionControlRod {
    ...
    public TileFissionInterface() {
        super(NuclearScienceTiles.TILE_FISSIONINTERFACE.get());
    }
    ...
    public void tickServer(ComponentTickable tickable) {
        ...
        if (!isInsertingFuelCell) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (item.isEmpty()) {
                } else if (item.getItem().is(NuclearScienceTags.Items.FUELROD_SPENT)) {
        ...
        if (!isInsertingDeuterium && !coreInv.areOutputsEmpty()) {
            ...
            for (int j = 9; j < 18; j++) {
                ...
                } else if (destItem.getItem().is(NuclearScienceTags.Items.CELL_TRITIUM) && destItem.getCount() < destItem.getMaxStackSize()) {
        ...
        if (!isExtractingSpentCell && !supplyInv.areInputsEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (item.getItem().is(NuclearScienceTags.Items.FUELROD_URANIUM_LOW_EN) || item.getItem().is(NuclearScienceTags.Items.FUELROD_URANIUM_HIGH_EN) || item.getItem().is(NuclearScienceTags.Items.FUELROD_PLUTONIUM)) {
                    continue;
                    ...
                    for (int j = 0; j < 9; j++) {
                        ...
                        if (supplyItem.getItem().is(NuclearScienceTags.Items.FUELROD_URANIUM_LOW_EN) || supplyItem.getItem().is(NuclearScienceTags.Items.FUELROD_URANIUM_HIGH_EN) || supplyItem.getItem().is(NuclearScienceTags.Items.FUELROD_PLUTONIUM)) {
        ...
        if (!isExtractingTritium && (deuterium.isEmpty() || (deuterium.getItem().is(NuclearScienceTags.Items.CELL_DEUTERIUM) && deuterium.getCount() < deuterium.getMaxStackSize()))) {
            ...
            for (int j = 0; j < 9; j++) {
                ...
                if (deuterium.getItem().is(NuclearScienceTags.Items.CELL_DEUTERIUM) && deuterium.getCount() >= deuterium.getMaxStackSize()) {
                    break;
                }

                item = supplyInv.getItem(j).copy();

                if (!item.getItem().is(NuclearScienceTags.Items.CELL_DEUTERIUM)) {
                    continue;
                }
