...
public class TileMSRFuelPreProcessor extends GenericTile {
	...
	public TileMSRFuelPreProcessor(BlockPos pos, BlockState state) {
		...
		addComponent(new ComponentFluidHandlerMulti(this).setTanks(1, 1, new int[] { MAX_TANK_CAPACITY }, new int[] { MAX_TANK_CAPACITY }).setInputDirections(BlockEntityUtils.MachineDirection.BOTTOM).setRecipeType(NuclearScienceRecipies.MSR_FUEL_PREPROCESSOR_TYPE.get()));
		...
		addComponent(new ComponentProcessor(this).canProcess((component, procNumber) -> component.consumeBucket().canProcessFluidItem2ItemRecipe(procNumber, NuclearScienceRecipies.MSR_FUEL_PREPROCESSOR_TYPE.get())).process(ComponentProcessor::processFluidItem2ItemRecipe));
