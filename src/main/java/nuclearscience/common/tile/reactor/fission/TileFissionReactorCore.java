...
public class TileFissionReactorCore extends GenericTile {
    ...
    public TileFissionReactorCore() {
        super(NuclearScienceTiles.TILE_REACTORCORE.get());
    ...
    protected void tickServer(ComponentTickable tickable) {
        ...
        if (fuelCount.getValue() > 0) {
            ...
            if(level.getRandom().nextFloat() < 0.01F) {
                ...
                level.playSound(null, getBlockPos(), sound, SoundCategory.BLOCKS, 1.0F, 1.0F);
            }

            if (level.getLevelData().getGameTime() % 10 == 0 && temperature.getValue() > 100) {
                AxisAlignedBB bb = AxisAlignedBB.ofSize(4, 4, 4).move(getBlockPos().getX(), getBlockPos().getY(), getBlockPos().getZ());
            ...
            if (controlRodCache.valid() && controlRodCache.getSafe() instanceof IFissionControlRod) {

                insertion = ((IFissionControlRod) controlRodCache.getSafe()).getInsertion();
    ...
    public void meltdown() {
        ...
        Explosion actual = new Explosion(level, null, VoltaicDamageTypes.RADIATION, null, worldPosition.getX(), worldPosition.getY(), worldPosition.getZ(), 20, true, Mode.NONE);
        ...
        level.explode(null, worldPosition.getX(), worldPosition.getY(), worldPosition.getZ(), 20, Mode.BREAK);
    ...
    public void processFissReact(ComponentInventory inv) {
        ...
        if (cachedRecipes == null || cachedRecipes.isEmpty()) {
            ...
            cachedRecipes = VoltaicRecipe.findRecipesbyType(NuclearScienceRecipies.FISSION_REACTOR_TYPE, level);
        }
