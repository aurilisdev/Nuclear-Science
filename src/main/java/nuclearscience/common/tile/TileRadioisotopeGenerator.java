...
public class TileRadioisotopeGenerator extends GenericTile {
	...
	addComponent(new ComponentElectrodynamic(this, true, false)
		.voltage(NuclearConstants.RADIOISOTOPEGENERATOR_VOLTAGE).extractPower((x, y) -> TransferPack.EMPTY)
	...
	RadiationUtils.handleRadioactiveItems(this, inv, NuclearConstants.RADIO_GENATOR_RADIATION_RADIUS, true, 30,
		true, false);

	double currentOutput = input.getCount() * NuclearConstants.RADIOISOTOPEGENERATOR_OUTPUT_MULTIPLIER
		* radiation.amount();

	if (currentOutput > 0) {
	    TransferPack transfer = TransferPack.ampsVoltage(
		    currentOutput / (NuclearConstants.RADIOISOTOPEGENERATOR_VOLTAGE * 2.0),
		    NuclearConstants.RADIOISOTOPEGENERATOR_VOLTAGE);
	    if (output1.valid()) {
