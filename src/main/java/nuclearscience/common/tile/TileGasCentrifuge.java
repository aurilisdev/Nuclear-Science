...
public class TileGasCentrifuge extends GenericTile implements ITickableSound {
	...
	public boolean canProcess(ComponentProcessor processor, int procNumber) {
		...
		RadiationUtils.handleRadioactiveGases(this, gasHandler, NuclearConstants.GAS_CENTRIFUGE_RADIATION_RADIUS, true, 1, true, false);
	...
	public void process(ComponentProcessor processor, int procNumber) {
		...
		if (VoltaicRegistries.gasRegistry().tags().getTag(NuclearScienceTags.Gases.URANIUM_HEXAFLUORIDE).contains(tank.getGas().getGas()) && tank.getGasAmount() >= processed) {
