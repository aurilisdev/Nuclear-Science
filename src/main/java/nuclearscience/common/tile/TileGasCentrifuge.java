...
public class TileGasCentrifuge extends GenericTile implements ITickableSound {
	...
	addComponent(new ComponentElectrodynamic(this, false, true).voltage(VoltaicCapabilities.DEFAULT_VOLTAGE * 2)
		...
		.maxJoules(NuclearConstants.GASCENTRIFUGE_USAGE_PER_TICK * 10));
	...
	RadiationUtils.handleRadioactiveGases(this, gasHandler, NuclearConstants.GAS_CENTRIFUGE_RADIATION_RADIUS, true,
		30, true, false);
	RadiationUtils.handleRadioactiveItems(this, inv, NuclearConstants.GAS_CENTRIFUGE_RADIATION_RADIUS, true, 30,
		true, false);
	...
	if (VoltaicRegistries.gasRegistry().tags().getTag(NuclearScienceTags.Gases.URANIUM_HEXAFLUORIDE)
		.contains(tank.getGas().getGas()) && tank.getGasAmount() >= processed) {
