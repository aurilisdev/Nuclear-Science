...
public class TileGasCentrifuge extends GenericTile implements ITickableSound {
	...
	public TileGasCentrifuge() {
		super(NuclearScienceTiles.TILE_GASCENTRIFUGE.get());
	...
	public boolean canProcess(ComponentProcessor processor, int procNumber) {
		...
		RadiationUtils.handleRadioactiveFluids(this, fluidHandler, NuclearConstants.GAS_CENTRIFUGE_RADIATION_RADIUS, true, 1, true, false);
		RadiationUtils.handleRadioactiveItems(this, inv, NuclearConstants.GAS_CENTRIFUGE_RADIATION_RADIUS, true, 1, true, false);
	...
	public void process(ComponentProcessor processor, int procNumber) {
		...
		if (FluidTags.getAllTags().getTag(NuclearScienceTags.Fluids.URANIUM_HEXAFLUORIDE.getName()).contains(tank.getFluid().getFluid()) && tank.getFluidAmount() >= processed) {
