...
public class TileRadioactiveProcessor extends GenericTile {
    ...
    public TileRadioactiveProcessor() {
        ...
        addComponent(new ComponentFluidHandlerMulti(this).setInputTanks(1, MAX_TANK_CAPACITY).setInputDirections(BlockEntityUtils.MachineDirection.TOP).setRecipeType(NuclearScienceRecipies.RADIOACTIVE_PROCESSOR_TYPE));
    ...
    private boolean shouldProcessRecipe(ComponentProcessor component, int procNumber) {
        ...
        boolean canProcess = component.canProcessFluidItem2ItemRecipe(procNumber, NuclearScienceRecipies.RADIOACTIVE_PROCESSOR_TYPE);
        if (BlockEntityUtils.isLit(this) ^ canProcess) {
            BlockEntityUtils.updateLit(this, canProcess);
        }

        RadiationUtils.handleRadioactiveFluids(this, (ComponentFluidHandlerMulti) getComponent(IComponentType.FluidHandler), NuclearConstants.RADIOACTIVE_PROCESSOR_RADIATION_RADIUS, true, 1, true, false);
