...
public class TileRadioactiveProcessor extends GenericTile {
	...
	public TileRadioactiveProcessor(BlockPos pos, BlockState state) {
		...
		addComponent(new ComponentElectrodynamic(this).voltage(ElectrodynamicsCapabilities.DEFAULT_VOLTAGE * 4).maxJoules(Constants.RADIOACTIVEPROCESSOR_USAGE_PER_TICK * 10.0).relativeInput(Direction.NORTH));
		addComponent(new ComponentFluidHandlerMulti(this).setAddFluidsValues(NuclearScienceRecipeInit.RADIOACTIVE_PROCESSOR_TYPE, MAX_TANK_CAPACITY, true, false).input(Direction.UP));
		addComponent(new ComponentInventory(this).size(6).inputs(1).outputs(1).bucketInputs(1).upgrades(3).processors(1).processorInputs(1).valid(machineValidator()).faceSlots(Direction.UP, 0).faceSlots(Direction.DOWN, 1).slotFaces(2, Direction.SOUTH, Direction.NORTH, Direction.EAST, Direction.WEST));
