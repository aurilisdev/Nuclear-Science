...
public class TileSteamFunnel extends GenericTile implements ISteamReceiver {
  ...
  public TileSteamFunnel(BlockPos worldPos, BlockState blockState) {
    ...
    addComponent(
        new ComponentTickable(this).tickServer(this::tickServer).tickClient(this::tickClient));
    addComponent(new ComponentPacketHandler(this));
    addComponent(new ComponentGasHandlerSimple(this, "storedsteam", INTERNAL_CAPACITY,
        MAX_TEMPERATURE, MAX_PRESSURE).setInputDirections(Direction.DOWN)
            .setOutputDirections(Direction.UP).setValidFluidTags(ElectrodynamicsTags.Gases.STEAM));
  }
  ...
  private void tickServer(ComponentTickable tickable) {
    ...
    if (level.getBlockEntity(getBlockPos().above()) instanceof ISteamReceiver receiver) {
      handler.drain(
          receiver.receiveSteam(handler.getGas().getTemperature(), handler.getGasAmount()),
          GasAction.EXECUTE);
    }
  ...
  private void tickClient(ComponentTickable tickable) {
    ...
    if (!handler.isEmpty() && level.getBlockEntity(above) instanceof ISteamReceiver
      ...
      double offsetFX =
          above.getX() + level.random.nextDouble() / 2.0 * (level.random.nextBoolean() ? -1 : 1);
      double offsetFY =
          above.getY() + level.random.nextDouble() / 2.0 * (level.random.nextBoolean() ? -1 : 1);
      double offsetFZ =
          above.getZ() + level.random.nextDouble() / 2.0 * (level.random.nextBoolean() ? -1 : 1);
  ...
  public double receiveSteam(double temperature, double amount) {
    ...
    return ((ComponentGasHandlerSimple) getComponent(IComponentType.GasHandler))
        .fill(new GasStack(ElectrodynamicsGases.STEAM.get(), amount, temperature,
            Gas.PRESSURE_AT_SEA_LEVEL), GasAction.EXECUTE);
  }
