...
public class TileParticleInjector extends GenericTile {
	...
	addComponent(new ComponentInventory(this, ComponentInventory.InventoryBuilder.newInv().inputs(2).outputs(1))
		...
		.setSlotsByDirection(BlockEntityUtils.MachineDirection.TOP, 0, 1)
		//
		.setSlotsByDirection(BlockEntityUtils.MachineDirection.RIGHT, 0, 1).setDirectionsBySlot(2,
	...
	addComponent(new ComponentElectrodynamic(this, false, true).voltage(VoltaicCapabilities.DEFAULT_VOLTAGE * 8)
		...
		.maxJoules(NuclearConstants.PARTICLEINJECTOR_USAGE_PER_PARTICLE * 2));
	...
	RadiationUtils.handleRadioactiveItems(this, (ComponentInventory) getComponent(IComponentType.Inventory),
		NuclearConstants.PARTICLE_INJECTOR_RADIATION_RADIUS, true, 30, true, false);
	...
	EntityParticle particle = new EntityParticle(
		dir, level, new Location(worldPosition.getX() + 0.5f + dir.getStepX(),
			worldPosition.getY() + 0.5f + dir.getStepY(), worldPosition.getZ() + 0.5f + dir.getStepZ()),
		getBlockPos());
	...
	if (!level.isClientSide()) {
	    ...
	    if (!cellStack.isEmpty() && resultStack.getCount() < resultStack.getMaxStackSize()
		    && one.speed >= EntityParticle.MIN_COLLISION_SPEED
		    && two.speed >= EntityParticle.MIN_COLLISION_SPEED) {
