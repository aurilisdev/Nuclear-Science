...
public class NuclearSciencePlugin implements IModPlugin {
	...
	registration.addRecipeCatalyst(GasCentrifugeRecipeCategory.INPUT_MACHINE,
		GasCentrifugeRecipeCategory.RECIPE_TYPE);
	registration.addRecipeCatalyst(NuclearBoilerRecipeCategory.INPUT_MACHINE,
		NuclearBoilerRecipeCategory.RECIPE_TYPE);
	registration.addRecipeCatalyst(ChemicalExtractorRecipeCategory.INPUT_MACHINE,
		ChemicalExtractorRecipeCategory.RECIPE_TYPE);
	registration.addRecipeCatalyst(FissionReactorRecipeCategory.INPUT_MACHINE,
		FissionReactorRecipeCategory.RECIPE_TYPE);
	registration.addRecipeCatalyst(ParticleAcceleratorAntiMatterRecipeCategory.INPUT_MACHINE,
		ParticleAcceleratorAntiMatterRecipeCategory.RECIPE_TYPE);
	registration.addRecipeCatalyst(ParticleAcceleratorDarkMatterRecipeCategory.INPUT_MACHINE,
		ParticleAcceleratorDarkMatterRecipeCategory.RECIPE_TYPE);
	registration.addRecipeCatalyst(FuelReprocessorRecipeCategory.INPUT_MACHINE,
		FuelReprocessorRecipeCategory.RECIPE_TYPE);
	registration.addRecipeCatalyst(RadioactiveProcessorRecipeCategory.INPUT_MACHINE,
		RadioactiveProcessorRecipeCategory.RECIPE_TYPE);
	registration.addRecipeCatalyst(MSRProcessorRecipeCategory.INPUT_MACHINE,
		MSRProcessorRecipeCategory.RECIPE_TYPE);
    }
	...
	// Gas Centrifuge
	List<PsuedoGasCentrifugeRecipe> gasCentrifugeRecipes = new ArrayList<>(
		NuclearSciencePsuedoRecipes.GAS_CENTRIFUGE_RECIPES);
	registration.addRecipes(GasCentrifugeRecipeCategory.RECIPE_TYPE, gasCentrifugeRecipes);

	// Nuclear Boiler
	List<NuclearBoilerRecipe> nuclearBoilerRecipes = recipeManager
		.getAllRecipesFor(NuclearScienceRecipies.NUCLEAR_BOILER_TYPE.get());
	...
	List<ChemicalExtractorRecipe> chemicalExtractorRecipes = recipeManager
		.getAllRecipesFor(NuclearScienceRecipies.CHEMICAL_EXTRACTOR_TYPE.get());
	registration.addRecipes(ChemicalExtractorRecipeCategory.RECIPE_TYPE, chemicalExtractorRecipes);

	// Fission Reactor
	List<FissionReactorRecipe> fissionReactorRecipes = recipeManager
		.getAllRecipesFor(NuclearScienceRecipies.FISSION_REACTOR_TYPE.get());
	registration.addRecipes(FissionReactorRecipeCategory.RECIPE_TYPE, fissionReactorRecipes);

	// Anti-Matter
	List<PsuedoItem2ItemRecipe> antiMatterRecipes = new ArrayList<>(
		NuclearSciencePsuedoRecipes.ANTI_MATTER_RECIPES);
	registration.addRecipes(ParticleAcceleratorAntiMatterRecipeCategory.RECIPE_TYPE, antiMatterRecipes);

	// Dark Mattere
	List<PsuedoItem2ItemRecipe> darkMatterRecipes = new ArrayList<>(
		NuclearSciencePsuedoRecipes.DARK_MATTER_RECIPES);
	registration.addRecipes(ParticleAcceleratorDarkMatterRecipeCategory.RECIPE_TYPE, darkMatterRecipes);

	// Fuel Reprocessor
	List<FuelReprocessorRecipe> fuelReprocessorRecipes = recipeManager
		.getAllRecipesFor(NuclearScienceRecipies.FUEL_REPROCESSOR_TYPE.get());
	registration.addRecipes(FuelReprocessorRecipeCategory.RECIPE_TYPE, fuelReprocessorRecipes);

	// Radioactive Processor
	List<RadioactiveProcessorRecipe> radioactiveProcessorRecipes = recipeManager
		.getAllRecipesFor(NuclearScienceRecipies.RADIOACTIVE_PROCESSOR_TYPE.get());
	registration.addRecipes(RadioactiveProcessorRecipeCategory.RECIPE_TYPE, radioactiveProcessorRecipes);

	// MSR Processor
	List<MSRFuelPreProcessorRecipe> msrProcessorRecipes = recipeManager
		.getAllRecipesFor(NuclearScienceRecipies.MSR_FUEL_PREPROCESSOR_TYPE.get());
	...
	registry.addRecipeClickArea(ScreenFissionReactorCore.class, 77, 38, 22, 15,
		FissionReactorRecipeCategory.RECIPE_TYPE);
	registry.addRecipeClickArea(ScreenParticleInjector.class, 102, 43, 28, 14,
		ParticleAcceleratorAntiMatterRecipeCategory.RECIPE_TYPE,
		ParticleAcceleratorDarkMatterRecipeCategory.RECIPE_TYPE);
	registry.addRecipeClickArea(ScreenMSRFuelPreProcessor.class, 98, 40, 16, 16,
		MSRProcessorRecipeCategory.RECIPE_TYPE);
    }
	...
	for (RegistryObject<? extends Fluid> fluid : NuclearScienceFluids.FLUIDS.getEntries()) {
	...
	for (RegistryObject<? extends Gas> gas : NuclearScienceGases.GASES.getEntries()) {
