...
public class NuclearSciencePlugin implements IModPlugin {
	...
	List<NuclearBoilerRecipe> nuclearBoilerRecipes = recipeManager
		.getAllRecipesFor(NuclearScienceRecipies.NUCLEAR_BOILER_TYPE.get()).stream().map(RecipeHolder::value)
		.toList();
	...
	List<FissionReactorRecipe> fissionReactorRecipes = recipeManager
		.getAllRecipesFor(NuclearScienceRecipies.FISSION_REACTOR_TYPE.get()).stream().map(RecipeHolder::value)
		.toList();
	...
	List<FuelReprocessorRecipe> fuelReprocessorRecipes = recipeManager
		.getAllRecipesFor(NuclearScienceRecipies.FUEL_REPROCESSOR_TYPE.get()).stream().map(RecipeHolder::value)
		.toList();
	...
	List<RadioactiveProcessorRecipe> radioactiveProcessorRecipes = recipeManager
		.getAllRecipesFor(NuclearScienceRecipies.RADIOACTIVE_PROCESSOR_TYPE.get()).stream()
		.map(RecipeHolder::value).toList();
	...
	List<MSRFuelPreProcessorRecipe> msrProcessorRecipes = recipeManager
		.getAllRecipesFor(NuclearScienceRecipies.MSR_FUEL_PREPROCESSOR_TYPE.get()).stream()
		.map(RecipeHolder::value).toList();
