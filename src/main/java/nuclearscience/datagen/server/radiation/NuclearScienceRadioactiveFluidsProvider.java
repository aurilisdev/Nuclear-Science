...
public class NuclearScienceRadioactiveFluidsProvider implements DataProvider {
    ...
    public void addFluid(Fluid fluid, double radiationAmount, double radiationStrength, JsonObject json) {
        ...
        json.add(BuiltInRegistries.FLUID.getKey(fluid).toString(), (JsonElement) RadioactiveObject.CODEC.encode(new RadioactiveObject(radiationStrength, radiationAmount), JsonOps.INSTANCE, data).result().get());
    }

    public void addTag(TagKey<Fluid> tag, double radiationAmount, double radiationStrength, JsonObject json) {
        JsonObject data = new JsonObject();
        json.add("#" + tag.location().toString(), (JsonElement) RadioactiveObject.CODEC.encode(new RadioactiveObject(radiationStrength, radiationAmount), JsonOps.INSTANCE, data).result().get());
    }
