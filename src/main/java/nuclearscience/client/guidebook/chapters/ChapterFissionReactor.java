...
public class ChapterFissionReactor extends Chapter {
	...
	@Override
	public void addData() {
		pageData.add(new TextWrapperObject(NuclearTextUtils.guidebook("chapter.fissionreactor.l1.1", NuclearScienceItems.ITEMS_NUCLEARMACHINE.getValue(SubtypeNuclearMachine.fissionreactorcore).getDescription().copy().withStyle(TextFormatting.BOLD))).setIndentions(1));
		...
		pageData.add(new TextWrapperObject(NuclearTextUtils.guidebook("chapter.fissionreactor.l6", NuclearScienceItems.ITEMS_NUCLEARMACHINE.getValue(SubtypeNuclearMachine.fissioncontrolrod).getDescription().copy().withStyle(TextFormatting.BOLD))).setIndentions(1).setSeparateStart());
