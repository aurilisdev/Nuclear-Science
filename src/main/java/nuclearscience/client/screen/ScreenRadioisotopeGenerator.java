...
public class ScreenRadioisotopeGenerator extends GenericScreen<ContainerRadioisotopeGenerator> {
	...
	private List<? extends IReorderingProcessor> getEnergyInformation() {
		...
		double currentOutput = in.getCount() * NuclearConstants.RADIOISOTOPEGENERATOR_OUTPUT_MULTIPLIER * rad.amount();
		TransferPack transfer = TransferPack.ampsVoltage(currentOutput / NuclearConstants.RADIOISOTOPEGENERATOR_VOLTAGE, NuclearConstants.RADIOISOTOPEGENERATOR_VOLTAGE);
		list.add(NuclearTextUtils.gui("machine.current", ChatFormatter.getChatDisplayShort(transfer.getAmps(), DisplayUnits.AMPERE)).withStyle(TextFormatting.GRAY).withStyle(TextFormatting.DARK_GRAY).getVisualOrderText());
		list.add(NuclearTextUtils.gui("machine.output", ChatFormatter.getChatDisplayShort(transfer.getWatts(), DisplayUnits.WATT).withStyle(TextFormatting.GRAY)).withStyle(TextFormatting.DARK_GRAY).getVisualOrderText());
		list.add(NuclearTextUtils.gui("machine.voltage", ChatFormatter.getChatDisplayShort(transfer.getVoltage(), DisplayUnits.VOLTAGE).withStyle(TextFormatting.GRAY)).withStyle(TextFormatting.DARK_GRAY).getVisualOrderText());
		return list;
	}
