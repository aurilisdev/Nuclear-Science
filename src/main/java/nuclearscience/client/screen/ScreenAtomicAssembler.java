...
public class ScreenAtomicAssembler extends GenericScreen<ContainerAtomicAssembler> {
	...
	addComponent(new ScreenComponentElectricInfo(-AbstractScreenComponentInfo.SIZE + 1, 2)
		.wattage(NuclearConfig.INSTANCE.ATOMICASSEMBLER_USAGE_PER_TICK.get() * 20));
	imageHeight += 64;
	inventoryLabelY += 64;
	addComponent(new ScreenComponentProgress(ScreenComponentProgress.ProgressBars.PROGRESS_ARROW_RIGHT, () -> {
	    ...
	    if (assembler != null) {
		return assembler.progress.getValue()
			/ (double) NuclearConfig.INSTANCE.ATOMICASSEMBLER_REQUIRED_TICKS.get();
	    }
	    return 0;
	}, 84, 71));
