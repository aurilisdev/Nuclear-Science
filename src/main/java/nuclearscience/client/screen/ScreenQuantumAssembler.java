...
public class ScreenQuantumAssembler extends GenericScreen<ContainerQuantumAssembler> {
	...
	private List<? extends FormattedCharSequence> getEnergyInformation() {
		...
		if (box != null) {
			...
			list.add(new TranslatableComponent("gui.quantumassembler.usage",
					new TextComponent(ChatFormatter.getElectricDisplayShort(Constants.QUANTUMASSEMBLER_USAGE_PER_TICK * 20, ElectricUnit.WATT))
							.withStyle(ChatFormatting.GRAY)).withStyle(ChatFormatting.DARK_GRAY).getVisualOrderText());
			list.add(new TranslatableComponent("gui.quantumassembler.voltage",
