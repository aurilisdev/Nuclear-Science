...
public class ScreenMSReactorCore extends GenericScreen<ContainerMSReactorCore> {
	...
	addComponent(new ScreenComponentTemperature(() -> {
	    ...
	    MutableComponent text = ChatFormatter.getChatDisplayShort(core.temperature.getValue(),
	...
	addComponent(new ScreenComponentMultiLabel(0, 0, graphics -> {
		...
		if (System.currentTimeMillis() % 1000 < 500) {
		    graphics.drawString(font, NuclearTextUtils.gui("msreactor.warning.overheat"), titleLabelX + 5,
			    titleLabelY + 48, WARNING_COLOR.color(), false);
		} else {
		    graphics.drawString(font, NuclearTextUtils.gui("msreactor.warning.overheat"), titleLabelX + 5,
			    titleLabelY + 48, Color.TEXT_GRAY.color(), false);
		}
