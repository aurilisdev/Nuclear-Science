	...
	public ScreenMoltenSaltSupplier(ContainerMoltenSaltSupplier container, PlayerInventory playerInventory, ITextComponent title) {
		...
		addComponent(new ScreenComponentProgress(ScreenComponentProgress.ProgressBars.PROGRESS_ARROW_RIGHT, () -> {
			...
			public void renderForeground(MatrixStack poseStack, int xAxis, int yAxis, int guiWidth, int guiHeight) {
				...
				renderTooltip(poseStack, NuclearTextUtils.gui("saltsupplier.wastecont", VoltaicTextUtils.ratio(ChatFormatter.formatFluidMilibuckets(supplier.reactorWaste.getValue()), ChatFormatter.formatFluidMilibuckets(TileMoltenSaltSupplier.AMT_PER_WASTE)).withStyle(TextFormatting.DARK_GRAY)).withStyle(TextFormatting.GRAY), xAxis, yAxis);
