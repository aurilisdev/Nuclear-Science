...
public class ScreenThermometerModule extends GenericInterfaceBoundScreen<ContainerThermometerModule> {
	...
	public ScreenThermometerModule(ContainerThermometerModule container, PlayerInventory inv, ITextComponent title) {
		...
		addComponent(new ScreenComponentCustomRender(0, 0, poseStack -> {
			...
			font.draw(poseStack, NuclearTextUtils.gui("logisticsnetwork.temperature", ChatFormatter.getChatDisplayShort(currTemp, DisplayUnits.TEMPERATURE_CELCIUS).withStyle(TextFormatting.GOLD)), guiWidth + 20, guiHeight + 45, Color.TEXT_GRAY.color());

			ITextComponent text = NuclearTextUtils.gui("logisticsnetwork.outputmode");
			...
			font.draw(poseStack, DisplayUnits.TEMPERATURE_CELCIUS.getSymbol().copy().withStyle(TextFormatting.WHITE), guiWidth + 20 + 120 + 2, guiHeight + 113, Color.TEXT_GRAY.color());

			font.draw(poseStack, NuclearTextUtils.gui("logisticsnetwork.signalstrength", new StringTextComponent("" + tile.redstoneSignal.getValue()).withStyle(TextFormatting.WHITE)), guiWidth + 20, guiHeight + 135, Color.TEXT_GRAY.color());
		...
		addComponent(modeButton = new ScreenComponentButton<>(20, 70, 68, 20).setLabel(() -> {
			...
			switch (TileThermometerModule.Mode.values()[tile.mode.getValue()]) {
			case BUILD_UP:
				return NuclearTextUtils.gui("logisticsnetwork.modebuildup");
			case CONSTANT:
				return NuclearTextUtils.gui("logisticsnetwork.modeconstant");
			default:
				return new StringTextComponent("");
			}
		}).setOnPress(button -> {
	...
	public void render(MatrixStack poseStack, int mouseX, int mouseY, float partialTicks) {
		...
		if (needsUpdate && getMenu().getSafeHost() instanceof TileThermometerModule) {
			targetTempBox.setValue(((TileThermometerModule) getMenu().getSafeHost()).targetTemperature.getValue() + "");
			needsUpdate = false;
		}
	}
