...
public class ScreenGasCentrifuge extends GenericMaterialScreen<ContainerGasCentrifuge> {
	...
	public ScreenGasCentrifuge(ContainerGasCentrifuge container, Inventory playerInventory, Component title) {
		...
		addComponent(new ScreenComponentMultiLabel(0, 0, poseStack -> {
			...
			font.draw(poseStack, Component.literal("U235 " + getIntString(centrifuge.stored235.getValue()) + "%"), 54, 17, 4210752);
			font.draw(poseStack, Component.literal("U238 " + getIntString(centrifuge.stored238.getValue()) + "%"), 54, 37, 4210752);
			font.draw(poseStack, Component.literal("DUST " + getIntString(centrifuge.storedWaste.getValue()) + "%"), 54, 58, 4210752);
		}));
	}
