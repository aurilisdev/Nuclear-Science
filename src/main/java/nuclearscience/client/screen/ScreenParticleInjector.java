...
public class ScreenParticleInjector extends GenericScreen<ContainerParticleInjector> {
    ...
    public ScreenParticleInjector(ContainerParticleInjector container, PlayerInventory playerInventory, ITextComponent title) {
        ...
        addComponent(new ScreenComponentElectricInfo(this::getElectricInformation, -AbstractScreenComponentInfo.SIZE + 1, 2).wattage(NuclearConstants.PARTICLEINJECTOR_USAGE_PER_PARTICLE));
        addComponent(new ScreenComponentButton<>(ScreenComponentGuiTab.GuiInfoTabTextures.REGULAR, -AbstractScreenComponentInfo.SIZE + 1, 2 * AbstractScreenComponentInfo.SIZE + 2)
                ...
                .onTooltip((poseStack, button, x, y) -> {
					...
					list.add(NuclearTextUtils.tooltip("particleinjector.gatewaymode").withStyle(TextFormatting.DARK_GRAY).getVisualOrderText());
					if(injector.usingGateway.getValue()) {
						list.add(NuclearTextUtils.tooltip("particleinjector.gatewayenabled").withStyle(TextFormatting.GREEN).getVisualOrderText());
					} else {
						list.add(NuclearTextUtils.tooltip("particleinjector.gatewaydisabled").withStyle(TextFormatting.RED).getVisualOrderText());
					}

					displayTooltips(poseStack, list, x, y);

                }));

        new WrapperInventoryIO(this, -AbstractScreenComponentInfo.SIZE + 1, AbstractScreenComponentInfo.SIZE + 2, 75, 82, 8, 72);

        addComponent(new ScreenComponentGeneric(ScreenComponentGuiTab.GuiInfoTabTextures.REGULAR_RIGHT, imageWidth - 1, 2).setIcon(NuclearJeiTextures.PARTICLEACCELERATOR_DMATOM).onTooltip((poseStack, component, x, y) -> {
            ...
            if(one != null && one.isAlive() && one.isAlive()) {
                oneSpeed = one.speed;
            }

            list.add(NuclearTextUtils.tooltip("particleinjector.particle1speed", ChatFormatter.getChatDisplayShort(TileElectromagneticGateway.getLightSpeedPerc(oneSpeed), DisplayUnits.PERCENTAGE).append(" ").append(NuclearDisplayUnits.SPEEDOFLIGHT.getSymbol()).withStyle(TextFormatting.GRAY)).withStyle(TextFormatting.DARK_GRAY).getVisualOrderText());

            EntityParticle two = injector.particles[1];

            float twoSpeed = 0.0F;

            if(two != null && two.isAlive() && two.isAlive()) {
                twoSpeed = two.speed;
            }

            list.add(NuclearTextUtils.tooltip("particleinjector.particle2speed", ChatFormatter.getChatDisplayShort(TileElectromagneticGateway.getLightSpeedPerc(twoSpeed), DisplayUnits.PERCENTAGE).append(" ").append(NuclearDisplayUnits.SPEEDOFLIGHT.getSymbol()).withStyle(TextFormatting.GRAY)).withStyle(TextFormatting.DARK_GRAY).getVisualOrderText());
    ...
    private List<? extends IReorderingProcessor> getElectricInformation() {
        ...
        list.add(NuclearTextUtils.tooltip("particleinjector.charge", ChatFormatter.getChatDisplayShort(el.getJoulesStored(), DisplayUnits.JOULES).withStyle(TextFormatting.GRAY), ChatFormatter.getChatDisplayShort(NuclearConstants.PARTICLEINJECTOR_USAGE_PER_PARTICLE, DisplayUnits.JOULES).withStyle(TextFormatting.GRAY)).withStyle(TextFormatting.DARK_GRAY).getVisualOrderText());
