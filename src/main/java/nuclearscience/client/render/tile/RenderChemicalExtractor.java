...
public class RenderChemicalExtractor implements BlockEntityRenderer<TileChemicalExtractor> {
	...
	@Override
	public void render(TileChemicalExtractor tileEntityIn, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
		...
		float prog = tileEntityIn.<ComponentFluidHandlerMulti>getComponent(ComponentType.FluidHandler).getTankFromFluid(Fluids.WATER, true).getFluidAmount() / (float) TileChemicalExtractor.MAX_TANK_CAPACITY;
		if (prog > 0) {
