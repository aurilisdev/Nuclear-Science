...
public class RenderFusionReactorCore extends AbstractTileRenderer<TileFusionReactorCore> {
	...
	if (Minecraft.getInstance().player.distanceToSqr(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5) <= 32) {
	    ...
	    renderFloatingText(matrixStackIn, bufferIn,
		    NuclearTextUtils.tooltip("tritiumlevel", tileEntityIn.tritium.getValue()), 0.5f, 0.3f + 1, 0.5f,
		    Color.WHITE.color(), combinedLightIn);
	...
	matrixStackIn.mulPose(new Quaternionf().rotationYXZ((float) Math.toRadians(camera.getYRot()),
		(float) Math.toRadians(-camera.getXRot()), 0.0F));
	...
	font.drawInBatch(text, xOffset, 0, color, false, matrix4f, bufferIn, DisplayMode.NORMAL, backgroundColor,
		combinedLightIn);
	...
	return super.getRenderBoundingBox(blockEntity);// AABB.encapsulatingFullBlocks(blockEntity.getBlockPos().above(),
