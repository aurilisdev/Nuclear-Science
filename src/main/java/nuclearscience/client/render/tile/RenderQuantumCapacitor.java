...
public class RenderQuantumCapacitor implements BlockEntityRenderer<TileQuantumCapacitor> {
	...
	public void render(TileQuantumCapacitor tileEntityIn, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn,
		...
		RenderingUtils.renderStar(matrixStackIn, bufferIn, tileEntityIn.getLevel().getLevelData().getDayTime(), (int) (250 / distance),
				tileEntityIn.getLevel().random.nextFloat() * 0.2f + 0.2f, 0, 0, 1, false);
		RenderingUtils.renderStar(matrixStackIn, bufferIn, tileEntityIn.getLevel().getLevelData().getDayTime() + 20f, (int) (250 / distance),
				tileEntityIn.getLevel().random.nextFloat() * 0.1f + 0.4f, 0, 0, 1, false);
		RenderingUtils.renderStar(matrixStackIn, bufferIn, tileEntityIn.getLevel().getLevelData().getDayTime() + 40f, (int) (250 / distance),
