...
public class RenderTurbine extends AbstractTileRenderer<TileTurbine> {
    ...
    @Override
    public void render(TileTurbine tileEntityIn, float partialTicks, PoseStack matrixStackIn,
	    MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
	boolean isCore = tileEntityIn.isCore.getValue();
	BakedModel ibakedmodel = Minecraft.getInstance().getModelManager()
		.getModel(NuclearScienceClientRegister.MODEL_TURBINEROTORLAYER);
	...
	return blockEntity.isCore.getValue() ? super.getRenderBoundingBox(blockEntity).inflate(1, 0, 1)
