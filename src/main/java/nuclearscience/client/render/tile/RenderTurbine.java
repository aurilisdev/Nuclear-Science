...
public class RenderTurbine extends TileEntityRenderer<TileTurbine> {
	...
	if (!isCore && tileEntityIn.getBlockState().getBlock().getRenderType(tileEntityIn.getBlockState()) != BlockRenderType.INVISIBLE) {
	    ...
	    Minecraft.getInstance().getBlockRendererDispatcher().getBlockModelRenderer().renderModelFlat(tileEntityIn.getWorld(), ibakedmodel,
	    ...
	    Minecraft.getInstance().getBlockRendererDispatcher().getBlockModelRenderer().renderModelFlat(tileEntityIn.getWorld(), ibakedmodel,
		    tileEntityIn.getBlockState(), tileEntityIn.getPos(), matrixStackIn, bufferIn.getBuffer(RenderType.getSolid()), true,
		    tileEntityIn.getWorld().rand, new Random().nextLong(), 1);
	    matrixStackIn.pop();
	} else if (isCore) {
	    float size = 3;
