...
public class RenderFissionReactorCore extends AbstractTileRenderer<TileFissionReactorCore> {
	...
	@Override
	public void render(TileFissionReactorCore tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
		BlockModelRenderer.enableCaching();
		IBakedModel fuelrod = Minecraft.getInstance().getModelManager().getModel(ClientRegister.MODEL_FISSIONREACTORFUELROD);
		...
		if (tileEntityIn.hasDeuterium.get()) {
			matrixStackIn.pushPose();
			IBakedModel deuterium = Minecraft.getInstance().getModelManager().getModel(ClientRegister.MODEL_FISSIONREACTORDEUTERIUM);
